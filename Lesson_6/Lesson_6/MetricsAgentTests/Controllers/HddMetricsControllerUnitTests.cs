    ...
    public class HddMetricsControllerUnitTests
    {
        ...
        public async Task GetMetricsByPeriod_ReturnOk()
        {
            ...
            _mockMediator.Setup(mediator => mediator.Send(It.IsAny<HddMetricGetByPeriodQuery>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(new AgentHddMetricResponse()
                {
                    Metrics = new List<AgentHddMetricDto>()
                    {
                       new AgentHddMetricDto()
                       {
                           Id = 1,
            ...
            var resultValue = ((OkObjectResult)result).Value as AgentHddMetricResponse;
        ...
        public void GetMetricsByPeriod_ShouldCall_LogInformation()
        {
            ...
            _mockMediator.Setup(mediator => mediator.Send(It.IsAny<HddMetricGetByPeriodQuery>(), It.IsAny<CancellationToken>())).ReturnsAsync(new AgentHddMetricResponse());
